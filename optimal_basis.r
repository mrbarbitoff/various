...
kallisto_summed = data.frame(kallisto_data$LENGTH)
...
covered = apply(kallisto_binary, 1, function(x) sum(x) > 0)
number_of_covered = sum(covered)
rownames(kallisto_binary) = tab[,1]

# Finding algo
kallisto_binary_filtered = kallisto_binary[covered, ]
contributors = c()
contributions = c()
for (i in 1:31){
  sums = apply(kallisto_binary_filtered, 2, sum)
  the_best_tissue = colnames(kallisto_binary_filtered)[which.max(sums)]
  print(the_best_tissue)
  contributors = c(contributors, the_best_tissue)
  contributions = c(contributions, max(sums))
  transcripts_to_keep = kallisto_binary_filtered[, the_best_tissue] == 0
  kallisto_binary_filtered = kallisto_binary_filtered[transcripts_to_keep, ]
  kallisto_binary_filtered = kallisto_binary_filtered[, -which.max(sums)]
}
contributions = c(contributions, sum(kallisto_binary_filtered))
contributors = c(contributors, levels(samples)[-which(levels(samples) %in% contributors)])
...
#Totalling
total_basis = data.frame(all_names = rownames(basis), all_counts = basis$number, prc_names = rownames(basis_prc), prc_counts = basis_prc$number)
